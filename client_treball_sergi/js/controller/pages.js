...
export async function pageTeams() {
  ...
  var teams = [];
  await Utils.loadJSON("./client_treball_sergi/json/teams_players.json").then((response) => (teams = JSON.parse(response)));
  teams.forEach((team) => {
...
export function pageRules() {
  ...
  Utils.showHideSearchBar(false);
  Utils.changeNavbarRightButtons();
  document.getElementById("page").innerHTML += '<iframe class="responsive-iframe" src="./client_treball_sergi/pdf.html" frameBorder="0"></iframe>';
...
export function pageTournaments() {
  ...
  var conversationalForm = window.cf.ConversationalForm.startTheConversation({
    ...
    userImage: "./client_treball_sergi/img/user.png",
    robotImage: "./client_treball_sergi/img/MLB_logo.png",
    dictionaryData: {
    ...
    flowStepCallback: async function (dto, success, error) {
      ...
      if (dto.tag.name == "tournament_type" && dto.tag.value[0] == Tournament.TOURNAMENT_TEAMS_SELECT_TYPE.ARBITRARY) {
        let _teams = [];
        await Utils.loadJSON("./client_treball_sergi/json/teams_players.json").then((response) => (_teams = JSON.parse(response)));
        let children = [];
